...
export const FleetContent = ({ fleet, onMediaLoad }: Props) => {
  ...
  return (
    <View
      className="relative h-full w-full overflow-hidden bg-light-surface dark:bg-dark-surface"
    ...
    >
      {fleet.media ? (
        ...
        >
          <UniImage
            source={{ uri: getCdnUrl({ src: fleet.media.url, width: containerWidth, variant: "small" }) }}
            style={{
              width: cqw(100),
      ...
      {fleet.stickers.map((sticker, index) => {
        ...
        return (
          ...
          >
            {sticker.imageUrl ? (
              <UniImage source={{ uri: sticker.imageUrl }} contentFit="contain" style={{ width: size, height: size }} />
            ) : (
